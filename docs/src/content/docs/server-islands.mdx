...
This integration provide a set of utilities to work with Astro's [Server Islands](https://docs.astro.build/en/guides/server-islands/).
...
This function will return `null` when not executing inside of a Server Island (running directly on a page with no `server:defer` in this component nor any parent component).

When running in the context of a Server Island, this function return an object with the following properties:
