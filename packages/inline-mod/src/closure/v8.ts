 ...
 * @internal
 */
export async function getFunctionLocation(func: Function) {
	// First, find the runtime's internal id for this function.
	const functionId = await getRuntimeIdForFunction(func);

	// Now, query for the internal properties the runtime sets up for it.
	const { internalProperties } = await runtimeGetProperties(functionId, /*ownProperties:*/ false);
 ...
 * @internal
 */
export async function lookupCapturedVariableValue(
	func: Function,
	freeVariable: string,
	...
	const scopesArray: { object?: Record<string, any> }[] = await getValueForObjectId(
		scopes.value.objectId
	);
...
async function createContext(): Promise<InflightContext> {
	...
	const session = <ContextSession>await v8Hooks.getSession();
...
async function getRuntimeIdForFunction(func: Function): Promise<inspector.Runtime.RemoteObjectId> {
	...
	const session = <EvaluationSession>await v8Hooks.getSession();
	const post = util.promisify(session.post);
...
async function runtimeGetProperties(
	objectId: inspector.Runtime.RemoteObjectId,
	ownProperties: boolean | undefined
) {
	const session = <GetPropertiesSession>await v8Hooks.getSession();
	const post = util.promisify(session.post);
...
async function getValueForObjectId(objectId: inspector.Runtime.RemoteObjectId): Promise<any> {
	...
	const session = <CallFunctionSession>await v8Hooks.getSession();
	const post = util.promisify(session.post);
	const context = await inflightContext();
	...
	if (retType.exceptionDetails) {
		throw new Error(
			`Error calling "Runtime.callFunction(${objectId})": ` + retType.exceptionDetails.text
		);
	}
