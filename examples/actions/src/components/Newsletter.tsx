...
export function Newsletter() {
	const [state, action, pending] = useActionState(withState(actions.subscribeToNewsletter.safe), {
		data: { success: false },
		error: undefined,
	});
	...
	return (
		<section className="bg-gray-900 -mx-4 rounded-2xl shadow-xl py-8 px-4 sm:px-8">
			...
			<p className="mb-8">Occasional posts and learnings from a lead Astro maintainer.</p>
				...
				<div className="flex flex-col gap-4 w-full">
					<div className="flex gap-4 flex-wrap">
						<div className="flex flex-col gap-2 flex-1">
							...
							/>
							{fieldErrors.email && <p className="text-red-300">{fieldErrors.email}</p>}
						</div>
						<button
			...
			{state?.error?.code === 'CONFLICT' && (
				<p className="bg-red-300 text-red-900 rounded px-2 py-1 mt-4">
					Oh, looks like you already subscribed. Check your inbox for a confirmation.
				</p>
			)}
