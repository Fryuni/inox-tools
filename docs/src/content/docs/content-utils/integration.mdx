...
Utility for integrations to declare their own content collections.
...
export default function yourIntegration(): AstroIntegration {
	return {
	...
	return {
		...
		hooks: {
			'astro:config:setup': (params) => {
				injectCollections(params, {
					entrypoint: '@my/package/collections',
					seedTemplateDirectory: new URL('./collectionTemplates', import.meta.url).toString(),
				});
			},
		},
	}
...
```ts title="integration/collection.ts" del={1-2} ins={3}
...
Utility for integrations to declare a seed for collections.
		...
		hooks: {
			'astro:config:setup': (params) => {
				seedCollections(params, {
					entrypoint: '@my/package/collections',
					templateDirectory: new URL('./collectionTemplates', import.meta.url).toString(),
				});
			},
		},
	}
