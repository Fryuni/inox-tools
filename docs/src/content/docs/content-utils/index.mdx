...
Each feature is also individually optimized such that whenever a trade-off is present, we optimize in favor of minimal runtime cost, moving as much of the processing to the initial built-time (before prerendering). This ensure that as your project grows the processing cost of this features to render your pages do not grow out of control.
