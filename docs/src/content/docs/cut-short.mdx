...
[^1]: The Sentry limitation is a bug on the integration and not a streaming limitation. Integrations can detect errors coming thrown from components even with streaming enabled, they just can't change the page status based on the error. This bug should be fixed soon.
