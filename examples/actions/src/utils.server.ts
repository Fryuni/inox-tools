...
export async function checkIfRateLimited(
	...
	const ip = import.meta.env.DEV ? scope('development') : ctx.locals.netlify.context.ip;
...
export async function getLikes({ postSlug }: { postSlug: string }): Promise<{ likes: number }> {
	const redis = Redis.fromEnv(getUpstashEnv());
